@php
    ...
    if($crud->model->translationEnabled()) {
        foreach((array) $filter->name as $filter_name){
            if($crud->model->isTranslatableAttribute($filter_name)) {
                $translatable = true;
            }
        }
        ...
        if(isset($filter->store_in) && $crud->model->isTranslatableAttribute($filter->store_in)) {
                $translatable = true;
        }
    }
